...
- **向导品类身份**：`page_number===1`=瓷砖（无名称/价格，标题行=品牌logo+品牌名）、`===2`=室内木门（材料管理隐藏，只走「门系列管理」单入口）；材料图片全品类必填；排序号冲突同子品类自动后移
- **门选材流程**：小程序先选门系列（单选+主图卡）再选该系列颜色；门材料仅 颜色/风格/图片 三字段全必填（前后端双校验）
