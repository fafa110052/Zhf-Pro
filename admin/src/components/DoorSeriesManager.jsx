...
/**
 * 门系列管理（通用组件）
 * pageNumber: 2=室内木门, 3=卫生间门
 */
export default function DoorSeriesManager({ pageNumber = 2, title = '门系列管理', description = '先维护通用颜色库，再到系列中挑选颜色并勾选所属风格' }) {
  ...
  const fetchSeries = useCallback(async (styleList) => {
    ...
    try {
      const res = await client.get('/admin/door-series', { params: { page_number: pageNumber } });
      const list = res.data || [];
      setSeries(list);
      if ((styleList || []).length) await fetchSeriesMaterials(list, styleList);
    } catch (err) { setError(err?.message || '加载失败'); }
    finally { setLoading(false); }
  }, [fetchSeriesMaterials, pageNumber]);

  useEffect(() => {
    (async () => {
      let styleList = [];
      try {
        const stRes = await client.get('/admin/styles');
        styleList = stRes.data || [];
        setStyles(styleList);
      } catch { /* 风格加载失败时矩阵为空 */ }
      fetchLibColors();
  ...
  const validateSeries = () => {
    ...
    if (!seriesForm.image_url.trim()) e.image_url = '请填写系列主图';
    setSeriesErrors(e);
  ...
  const handleSeriesSubmit = async (ee) => {
    ...
    try {
      const payload = { name: seriesForm.name.trim(), image_url: seriesForm.image_url.trim() || null, sort_order: Number(seriesForm.sort_order) || 0, page_number: pageNumber };
      if (seriesMode === 'add') {
  ...
  const validateLib = () => {
    ...
    if (!libForm.image_url.trim()) e.image_url = '请填写色块图';
    setLibErrors(e);
    return Object.keys(e).length === 0;
  };
