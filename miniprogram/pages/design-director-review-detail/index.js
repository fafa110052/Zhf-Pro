...
Page({
  data: {
    ...
    // 驳回弹窗
    rejectOpen: false, rejectReason: '', rejecting: false,
    acting: false,
  ...
  async onApprove() {
    wx.showModal({
      title: '确认通过', content: '确定审核通过该设计图吗？', confirmText: '通过',
      success: async (res) => {
  ...
  async onSubmitReject() {
    ...
    finally { this.setData({ rejecting: false }); }
  },

  onPreview(e) {
    const url = e.currentTarget.dataset.url;
    const urls = this.data.phase?.design_images || [url];
