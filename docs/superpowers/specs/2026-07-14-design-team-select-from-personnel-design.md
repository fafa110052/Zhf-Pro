...
2. 选中后自动**预填**该人的**姓名**与**头像**；两者**仍可继续编辑**——姓名可改，头像可重新粘贴链接或本地上传替换（保留现有头像输入框+上传按钮+预览）。
...
3. **弹窗 add 分支**（`dtFormMode === 'add'`）：在现有"姓名/头像"输入框**上方新增一个「人员下拉」**用于预填；姓名、头像、擅长风格、排序四个字段全部保持手动可编辑（头像输入框+本地上传+预览沿用现状）。
4. **弹窗 edit 分支**（`dtFormMode === 'edit'`）：完全保持现有手动字段，不显示人员下拉。
5. **下拉 onChange**：根据选中人 set `dtPersonId`、`dtName`、`dtAvatar`（仅预填，用户可再改）。
...
2. 选中一人 → 姓名、头像自动预填正确；预填后仍可手动改姓名、可重新上传/替换头像并即时预览。
